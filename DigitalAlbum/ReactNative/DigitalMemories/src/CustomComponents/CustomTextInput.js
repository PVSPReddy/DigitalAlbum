    ...
    const validationReducer = (state = initialState, action) => {
        if (action.type === CHECK_VALIDATION) {
            ...
            if (!isTextEmpty) {
                ...
                //^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[*.!@$%^&(){}[]:;<>,.?/~_+-=|\]).{8,32}$/
                if(action.isPasswordInput && !(mediumPasswordRegex.test(action.checkforValue)))
                {
                    isValidValue = false;
                }
                else if(action.isMobile && !(standardMobileRegex.test(action.checkforValue)))
                {
                    isValidValue = false;
                }
                else if(action.isEmail && !(standardEmailRegex.test(action.checkforValue)))
                {
                    isValidValue = false;
                }
            }
