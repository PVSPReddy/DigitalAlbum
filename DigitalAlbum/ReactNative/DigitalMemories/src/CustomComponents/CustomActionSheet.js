        ...
        const uiComponent = (
            <CustomTouch onPress={ () => this.onOptionSelectedHandler(item)}>
                ...
                <View style={styles.actionSheetItemsHolder}>
                    <Text style={{ ...styles.actionSheetItemsTextStyle, ...this.props.actionSheetItemsTextStyle }}>{item.text}</Text>
                </View>
            </CustomTouch>
        )
        return uiComponent;
        ...
        const mainUIComponent = (
            <Modal
                ...
                <View style={styles.actionSheetMainHolderStyle}>
                    <View style={{ ...styles.actionSheetDataHolderStyle, ...this.props.style }}>
                        <View style={{ ...styles.actionSheetHeaderHolderStyle, ...this.props.headerStyle }}>
                            <Text style={{ ...styles.actionSheetHeaderTextStyle, ...this.props.headerTextStyle }}>{this.props.title}</Text>
                        </View>
                        {/* <Text style={{ ...styles.actionSheetDescriptionTextStyle, ...this.props.descritionTextStyle }}>{this.props.description}</Text> */}
                        <View style={{ ...styles.actionSheetContentHolderStyle, ...this.props.bodyStyle }}>
                            {/* {props.children} */}
                            {this.dataItems.map(item => {
                                return this.createButtons(item);
                            })}
                        </View>
                        {this.createButtons({id: "cancel", text: "Cancel", onPress: this.props.onCancelPress })}
                    </View>
                </View>
            </Modal>
        )
